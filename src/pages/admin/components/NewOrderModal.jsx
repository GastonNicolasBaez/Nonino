...
export function NewOrderModal({ onClose, onSave }) {
    ...
    useEffect(() => {
        ...
        const filtered = inventarioProductosSucursal.filter(product => {
                const matchesSearch = !productSearch.trim() || 
                  product.name.toLowerCase().includes(productSearch.trim().toLowerCase()) ||
                  product.categoryName.toLowerCase().includes(productSearch.trim().toLowerCase());
